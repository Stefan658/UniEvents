...
CSV_PATH = "research/unievents_seed_candidates.csv"
...
def get_or_create_organizer(email, role_id, counters):
    ...
    if not user:
        ...
        first_name = parts[0].capitalize() if parts else "USV"
...
def seed_realistic_events():
    ...
    with app.app_context():
        print(f"--- Starting Realistic Seeding from {CSV_PATH} ---")
        ...
        try:
            with open(CSV_PATH, "r", encoding="utf-8") as f:
                ...
                for i, row in enumerate(reader, start=1):
                    ...
                    category = get_or_create_category(row.get('category_name'), counters)
                    ...
                    try:
                        ...
                        purpose = row.get('seed_purpose', '')
                        print(f"  + Added: {title} [{purpose}]")
